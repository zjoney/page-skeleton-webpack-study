...
class Server extends EventEmitter {
  ...
  async initRouters() {
    ...
    staticFiles
      .filter(file => /\.bundle/.test(file))//找到所有的 bundle也就是打包后的文件
      .forEach((file) => {//const staticPath = '__webpack_page_skeleton__'
        app.get(`/${staticPath}/${file}`, (req, res) => {
          res.setHeader('Content-Type', 'application/javascript')//返回这个脚本
          fs.createReadStream(path.join(__dirname, '..', 'client', file)).pipe(res)
        })
      })
    //如果是预览的话返回preview/dist/index.html  
    app.get('/preview.html', async (req, res) => {
      fs.createReadStream(path.resolve(__dirname, '..', 'preview/dist/index.html')).pipe(res)
    })

    app.get('/:filename', async (req, res) => {
      const { filename } = req.params //获取到文件名
      if (!/\.html$/.test(filename)) return false//如果不是html文件则返回
